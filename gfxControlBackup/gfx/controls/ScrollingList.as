...
[InspectableList("disabled", "visible", "itemRenderer", "inspectableScrollBar", "rowHeight", "inspectableRendererInstanceName", "margin", "paddingTop", "paddingBottom", "paddingLeft", "paddingRight", "thumbOffsetBottom", "thumbOffsetTop", "thumbSizeFactor", "enableInitCallback", "soundMap","scrollDisable")]
...
// UI Elements:
    ...
    private var _scrollDisable:Boolean=false
	//
...
// Public Methods:	
	...
	public function get scrollDisable():Boolean { return _scrollDisable; }
	public function set scrollDisable(value:Boolean):Void { 
		_scrollDisable = value;
		//if (initialized) { setState(); }
	}
...
// Private Methods:
	...
	private function scrollWheel(delta:Number):Void {
		//var sp:Number
		if(_scrollDisable){return;}
		if (_disabled) { return; }
		
		var pageScrollSize = (_scrollBar != undefined) ? _scrollBar.pageScrollSize : 1;
			if(delta>0){
				scrollPosition = _scrollPosition - (1 * Math.ceil(pageScrollSize));
			}else{			
				scrollPosition = _scrollPosition - (-1 * Math.ceil(pageScrollSize));
			}
		 //scrollPosition
		//trace ("scrollWheel delta = "+delta)
		//trace ("scrollWheel position = "+sp )
		
	}
	
	/*private function setTimePosition(arg){
