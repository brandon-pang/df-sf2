...
class com.scaleform.MsnMaskedList extends ScrollingList {
		
	private var itemContainer:MovieClip;
	private var mask:MovieClip;
	private var bg:MovieClip;

	public var _itemHeight:Number; //The height of the item renderer.
	public var _viewRendererCount:Number; //The number of renderers that will be displayed in the view.
	public var _drawnRendererCount:Number; //The number of drawn renderers (displayed + cached) in our list.
	public var _bottomFlatPixelShift:Number;  //The pixel shift so any bottom renderer offscreen is 100% displayed flat against the bottom.
	
	public var _lastPosition:Number; //The last _scrollPosition of the list. 
	public var _topRenderer:Number; //The index of the _topRenderer.
	
	public var _startIndex:Number; //Used for repopulating data for invalidated renderers.
	public var _endIndex:Number; //Used for repopulating data for invalidated renderers.
    public var _maxScrollNo:Number;
	private var _targetWidth:Number;

// Initialization:
	public function MsnMaskedList() { 
		super();
...
// Private Methods:
	...
	private function scrollPixel(scrollPos:Number):Void {
		...
		trace ("scrollPixel _scrollPosition " + _scrollPosition )
		//trace ("scrollPixel" + _lastPosition )
	}
	...
	private function updateScrollBar():Void {		
		...
		if (_scrollBar.setScrollProperties != null) {
			_maxScrollNo=(((totalRenderers-1 - Math.floor(_viewRendererCount)) * _itemHeight) + _bottomFlatPixelShift)
		...
		trace ("max" + max )
		trace ("_maxScrollNo = " + _scrollPosition )
	}
	...
	private function handleScroll(event:Object):Void {
		scrollPosition = Math.ceil(_scrollBar.position);
		scrollPixel(scrollPosition);
	}
