...
class com.scaleform.MaskedList2 extends ScrollingList {
...
// Private Methods:
	...
	private function repositionCheck(topIndex:Number):Void {
		...
		if (startIndex != -1) {
			...
			_dataProvider.requestItemRange((topIndex + startIndex), (topIndex + endIndex+1), this, "populateDataFromTo");
		}
	}
	...
	private function scrollWheel(delta:Number):Void {
		...
		if(delta>0){
			scrollPosition = (_scrollPosition) - (1 * Math.ceil(_scrollBar.pageScrollSize));
		}else{			
			scrollPosition = (_scrollPosition) - (-1 * Math.ceil(_scrollBar.pageScrollSize));			
		}
