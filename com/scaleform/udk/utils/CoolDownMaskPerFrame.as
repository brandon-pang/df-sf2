 ...
 * @author noww
 */
class com.scaleform.udk.utils.CoolDownMaskPerFrame
{
	...
	public function CoolDownMaskPerFrame(target:MovieClip, maskWidth:Number, maskHeight:Number, increaseCool:Boolean)
	{	
	...
	{
		var time:Number = _deltaTime;
		if (time >= _cooldownTime)
		{
			if (_increaseCool) { fullRectDraw(); }
			else { reset(); }
			dispatchEvent({type:"coolDownEnd"});
			return;
		}
		var tempTime:Number = Math.floor(time/_unitTime);
		if (_tempTime != tempTime)
		{
			if (!_increaseCool && time == 0) { fullRectDraw(); }
			else
			{
				if (!_increaseCool || time != 0) { update(time); }
			}
			_tempTime = tempTime;
		}
		dispatchEvent({type:"coolDownProgress", remainTime:_cooldownTime - time});
	}
