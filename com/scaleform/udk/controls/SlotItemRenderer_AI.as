...
[InspectableList("disabled", "visible", "labelID", "disableConstraints")]
 class com.scaleform.udk.controls.SlotItemRenderer_AI extends ListItemRenderer
{
	...
	//private var unitMc:MovieClip;

	//private var mannerMc:MovieClip;
	...
	private var smallClose:Button;
	private var _hit:MovieClip;
	private var AI_player_txt:MovieClip;
	private var ai_icon_mc:MovieClip;
	...
	// Initialization:
	public function SlotItemRenderer_AI()
	{
		super();
	...
	private function configUI():Void {
		...
		smallClose.addEventListener("click",this,"handlepaperPress");
	}
	...
	private function UpdateTextFields()
	{
		...
		ai_icon_mc._visible=false
		slotRank._visible = false;			
		slotRank.gotoAndPlay(1)

		if (_lock == 0)
		{
			AI_player_txt._x=1000
			smallClose._x=1000
			AI_player_txt._visible=false
			smallClose._visible=false
		...
		else if(_lock == 1)
		{
			...
			nonSlot._visible = false;
			numbericon._visible = false;
			AI_player_txt.htmlText="<font color='#666666'>_ai_waitroom_player_wait</font>"
			AI_player_txt._x=4
			smallClose._x=315
			AI_player_txt._visible=true
			ai_icon_mc._visible=false
			setVisible(false);
		}
		else if(_lock==3){
			...
			nonSlot._visible = false;
			numbericon._visible = false;
			AI_player_txt.htmlText="<font color='#ffffff'>_ai_waitroom_ai_wait</font>"			
			ai_icon_mc._visible=true
			AI_player_txt._x=20
			smallClose._x=315
			AI_player_txt._visible=true
		...
		if(data.isCloseBtn=="1"){
			smallClose._visible=true
		}else{
			smallClose._visible=false
		}
	...
	private function handleMouseDown(button, target, idx):Void
	{
		...
		if (_codename != "")
		{
			...
			_global.OnSlotListPress(Number(clickValue) - 1,button,data.Lock);
		...
		else
		{
			...
			_global.OnSlotBtnPress(Number(clickValue) - 1,button,data.Lock);
		}
	...
	private function handlepaperPress(mouseIndex:Number, button:Number):Void {
		var clickValue = this._name.substring(4);
		ExternalInterface.call("OnSlotClose",Number(clickValue) - 1,data.Lock);
	}
