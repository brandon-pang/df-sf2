...
class com.scaleform.udk.controls.PmcShopListItem extends ListItemRenderer {
	...
	private var _tagBoo:Boolean = false;
	...
	private var tagIconPos:Array = [120, 93, 66,36];
	...
	public function PmcShopListItem() {
		super();
	}
	...
	private function setTagIcon():Void
	{
		...
		if (_limitCase==null||_limitCase=="")
		{
			...
			buyLimitIcon._x=183/2-((10+buyLimitIcon["tagBg"]._width)/2)
		}
	}
	
	
	private function lvLoader() {
		var n:String=data.ItemIndex
		var imgPathArmor:String
		var imgPathWeapon:String;
		var imgPathCashItem:String;
		var imgPathUnit
		var imgName = data.IconImg;
		...
		if (n == "0") {
			mcLoader.loadClip(imgPathArmor,modeIcon);
		} else if (n == "1") {
			mcLoader.loadClip(imgPathWeapon,modeIcon);
		} else if (n == "2") {
			mcLoader.loadClip(imgPathUnit,modeIcon);
		} else if (n == "3") {
			mcLoader.loadClip(imgPathCashItem,modeIcon);
		}
	...
	{
		var _itemid:String = data.ItemIndex
	...
	private function onChkHandler(e:Object) {
		...
		data.Chk = selNo;
		_global.OnWeaponShopChkData(this._parent._parent._parent._id,this.index,_boo);
		trace("targetPath = "+_parent._parent._parent._parent._name);
	...
	private function onDblChkHandler(e:Object) {
		...
		_global.OnWeaponShopChkDblData(this._parent._parent._parent._id,this.index);
	}
	...
	private function onChkOverHandler(e:Object) {
		...
		//툴팁노출
		if (_Limitedclass != "" && _Limitedclass != undefined) {
			_level0.setLimitedTipView(_Limitedclass,(arrowPoz.x+30),arrowPoz.y);
		}
	}
		...
		for (var i = 0; i<4; i++) {
			for (var a = 0; a<owner.dataProvider.length; a++) {
				if (i == preNo && a == nowNo) {
					...
					_parent._parent._parent._parent["renderer"+i].charShopItemList.container["renderer"+a].data.Chk = 0;
					_parent._parent._parent._parent["renderer"+i].charShopItemList.container["renderer"+a].selectMc.gotoAndPlay(1);
				}
